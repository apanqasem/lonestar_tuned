...
//#endif
	...
	for (unsigned ii = 0; ii < nnodes; ++ii) {
		...
		for (unsigned jj = 0; jj < noutgoing[ii]; ++jj) {
			...
			// commented out the following line, used for edge weight calcualtion
			// it appears memory allocation is not done correctly when the graph is unweighted
			// memory alloc done in terms of "foru"? what is that?
			//   - @apanqasem 07/21/17 
